...
const App = () => {
    ...
    const indianCity = cityList.filter((element,index)=>(element.country === "India"));

    const listItems = indianCity.map((element,index)=>(
      <li key = {`location${index+1}`}>{element.name}</li>
    ));
